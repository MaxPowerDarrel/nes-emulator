...
pub fn decode(opcode: u8) -> Option<Instr> {
    ...
    let (name, mode, cycles, page_cycle, unofficial) = match opcode {
        ...
        0x1A | 0x3A | 0x5A | 0x7A | 0xDA | 0xFA => ("NOP", Implied, 2, false, true),
        // Immediate (2-byte, 2 cycles)
        0x80 | 0x82 | 0x89 | 0xC2 | 0xE2 => ("NOP", Immediate, 2, false, true),
        // Zero Page (2-byte, 3 cycles)
        0x04 | 0x44 | 0x64 => ("NOP", ZeroPage, 3, false, true),
        // Zero Page,X (2-byte, 4 cycles)
        0x14 | 0x34 | 0x54 | 0x74 | 0xD4 | 0xF4 => ("NOP", ZeroPageX, 4, false, true),
        // Absolute (3-byte, 4 cycles)
        0x0C => ("NOP", Absolute, 4, false, true),
        // Absolute,X (3-byte, 4 cycles + 1 page cross)
        0x1C | 0x3C | 0x5C | 0x7C | 0xDC | 0xFC => ("NOP", AbsoluteX, 4, true, true),
...
pub fn execute(cpu: &mut Cpu, bus: &mut Bus, _opcode: u8, instr: &Instr) -> u8 {
    ...
    match instr.name {
        ...
        // ── Compare ──────────────────────────────────────────────────────────
        "CMP" => {
            cpu.compare(bus.read(op.addr), cpu.a);
        }
        "CPX" => {
            cpu.compare(bus.read(op.addr), cpu.x);
        }
        "CPY" => {
            cpu.compare(bus.read(op.addr), cpu.y);
        }
